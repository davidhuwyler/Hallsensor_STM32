...
class AnalogIN {
public:
	  __attribute__((always_inline)) AnalogIN(const std::uint8_t portNr, const std::uint8_t pinNr) : portNumber(portNr),pinNumber(pinNr)
	  {
		  switch(portNr){
	  ...
	  inline void __attribute__((always_inline)) getValue()
	  {
